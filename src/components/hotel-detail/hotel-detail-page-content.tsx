...
export function HotelDetailPageContent({
  ...
  // Offre déjà en mémoire si l'utilisateur vient de la liste de résultats (cf. hotel-results.tsx),
  // sert de fallback tant que le fetch détail n'a pas encore répondu.
  ...
  // Synchronise le store avec les données fetchées, pour que la page de réservation
  // (étape suivante du tunnel) puisse lire hotelDetail/roomOffers sans re-fetcher.
  useEffect(() => {
  ...
  useEffect(() => {
    if (isError) {
      setError(t("roomsError") ?? "Impossible de charger les offres de chambres pour le moment.");
    }
  }, [isError, setError, t]);
  ...
  // Résolution dynamique des informations : priorité au fetch détail, puis au store
  // (rechargement de page où l'utilisateur vient de la liste), puis aux props initiales.
  const name = hotel?.name || selectedOffer?.hotelName || initialHotelName;
      ...
      <div className="container mx-auto max-w-7xl px-4 py-6 space-y-8">
        ...
        <section className="space-y-4 pt-4 border-t border-border/40">
          {isLoading ? (
              ...
              <HotelRoomList offerId={String(offerId)} roomOffers={rooms} nights={nights} isReseller={isReseller} />
          )}
        </section>
      </div>
  );
