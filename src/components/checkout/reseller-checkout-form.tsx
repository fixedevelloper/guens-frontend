...
const buildCheckoutRequestSchema = (isFlight: boolean) => z.object({
  contactEmail: z
      .string()
      .trim()
...
export type ResellerBookingCheckout = z.infer<ReturnType<typeof buildSchema>>;
  ...
  return (
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
              ...
              {fields.map((field, index) => (
                  ...
                  >
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      ...
                      <FormField
                          control={form.control}
                          name={`checkout.travelers.${index}.dateOfBirth`}
                          render={({ field }) => (
                              <FormItem>
                      ...
                      <FormField
                          control={form.control}
                          name={`checkout.travelers.${index}.passportNumber`}
                          render={({ field }) => (
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input
                                      ...
                                      className="rounded-xl border-border/80 bg-background focus-visible:ring-primary/20 uppercase"
                                      {...field}
                                  />
                                </FormControl>
                                <FormMessage />
                      ...
                      <FormField
                          control={form.control}
                          name={`checkout.travelers.${index}.nationality`}
                          render={({ field }) => (
                              <FormItem>
                      ...
                      <FormField
                          control={form.control}
                          name={`checkout.travelers.${index}.passportIssueCountry`}
                          render={({ field }) => (
                              <FormItem>
          ...
          {/* SECTION 3 : OPTIONS DE PAIEMENT & MONTANT REVENDEUR */}
                ...
                {(["PAY_NOW", "PAY_LATER"] as const).map((value) => {
                  ...
                  return (
                      <button
                          key={value}
                          type="button"
                          onClick={() => form.setValue("checkout.paymentPlan", value)}
                          className={cn(
          ...
          <div className="space-y-4 pt-4 border-t border-border/40">
            <p className="text-xs text-muted-foreground/80 leading-relaxed bg-slate-50 dark:bg-zinc-900/40 p-3.5 rounded-xl border border-border/30">
              {t("guestNotice") ?? "En soumettant cette demande, vous confirmez que les informations ci-dessus correspondent exactement aux pièces d'identité officielles de voyage."}
            </p>
