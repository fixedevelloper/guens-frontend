...
import { PassportImageRequirementPanel } from "@/components/checkout/passport-image-upload";
...
interface PaymentPageContentProps {
  bookingId: string;
  /** Where "Retour à la réservation" and the post-payment redirect land - the customer journey
   *  uses the public guest tracking page, the reseller journey stays under their own dashboard
   *  space (see dashboard/reseller/bookings/[bookingId]). */
  trackingPath: (bookingId: string) => string;
  /** Where the "booking not found" fallback links back to. */
  notFoundPath: string;
}

export function PaymentPageContent({ bookingId, trackingPath, notFoundPath }: PaymentPageContentProps) {
  ...
  function proceedToBookingTracking(payment: PaymentResponse) {
    ...
    router.push(trackingPath(bookingId));
  }
  ...
  useEffect(() => {
    if (
      ...
      router.replace(trackingPath(bookingId));
    }
    ...
    function handleSubmit(values: PaymentFormValues) {
        const basePayload = {
            ...
            // Seulement rempli en mode paiement manuel (voir PaymentForm/ManualPaymentPanel) - ignoré
            // par le backend pour tout autre provider.
            customerReference: values.customerReference?.trim() || undefined,
        };
    ...
    return (
        <div className="mx-auto max-w-xl px-4 py-16 text-center">
          ...
          <Button asChild variant="outline" className="mt-4 rounded-xl">
            <Link href={notFoundPath}>Retour au tableau de bord</Link>
          </Button>
        </div>
    );
  }
  ...
  const hasUnresolvedPassportImageRequirement = booking.travelers.some(
      (traveler) => traveler.passportImageRequired && !traveler.passportImageUrl
  );
  ...
  return (
      <div className="min-h-[calc(100vh-4rem)] max-w-5xl mx-auto px-4 py-8 lg:py-12">
        ...
        >
          <Link href={trackingPath(bookingId)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 rounded-2xl border border-border/50 bg-card p-6 sm:p-8 shadow-2xs space-y-6">
            ...
            {hasUnresolvedPassportImageRequirement ? (
                <PassportImageRequirementPanel bookingId={bookingId} travelers={booking.travelers} />
            ) : (
                <PaymentForm
